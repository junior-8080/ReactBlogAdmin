...
const StepModal  = ({isModalVisible,handleVisibility}) => {
    ...
    const onFinished = () => {
        ...
        fetch('http://localhost:3004/v1/api/posts',{
            method:'POST',
            headers: {
    ...
    return (
        ...
        footer={[
            <Button key="back" onClick={prev} disabled ={current < 0 || current !== steps.length -1} size="small">
              previous
            </Button>,
            <Button key="next" type="primary"  onClick = {next} disabled = {current === steps.length -1  && steps.length -1} size="small">
              next
            </Button>,
             <Button key="submit" type="primary" disabled = {current !== steps.length -1 || name.length < 5 || summary.length < 5} size="small" onClick={onFinished}>
                finished
            </Button>,
